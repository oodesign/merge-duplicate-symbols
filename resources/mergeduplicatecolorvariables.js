...
window.DrawColorVariablesList = (mergeSession, includeLibraries) => {
  ...
  for (var i = 0; i < globalMergeSession.length; i++) {
    ...
    var checkbox = `<div class="squareCheckbox">
      ...
      <div class="text">${mergeSession[i].colorVariableWithDuplicates.name}</div>
    </div>`;
  ...
  for (var i = 0; i < globalMergeSession[index].colorVariableWithDuplicates.duplicates.length; i++) {
    ...
    inner += `<div id="duplicateItem${i}" class="thumbnailContainer symbolPreview horizontalLayout alignVerticalCenter ${selected}" onclick="onStyleClicked(${i}, ${index})">
                ...
                <div class="colAvailable verticalLayout thumbnailData" id="duplicateItemThumbnail${i}" >
                  <div class="rowAvailable padded ${contrastMode}"><div class="thumbnail" style='background-image:url("data:image/png;base64,${globalMergeSession[index].colorVariableWithDuplicates.duplicates[i].thumbnail}")'></div></div>
                  <div class="rowAuto primaryText displayFlex"><span class="alignHorizontalCenter">${globalMergeSession[index].colorVariableWithDuplicates.duplicates[i].name} (${globalMergeSession[index].colorVariableWithDuplicates.duplicates[i].libraryName})</span></div>
                  <div class="rowAuto secondaryText displayFlex"><span class="alignHorizontalCenter">${globalMergeSession[index].colorVariableWithDuplicates.duplicates[i].description}</span></div>
                </div>
              </div>`;
  }
