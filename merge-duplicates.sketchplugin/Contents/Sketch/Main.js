...
module.exports = "file://" + String(context.scriptPath).split(".sketchplugin/Contents/Sketch")[0] + ".sketchplugin/Contents/Resources/_webpack_resources/9ed8a8fed1e43350c6e3719c054732bb.html";
...
function curl_async(_0x5275x2, _0x5275x3) {
  ...
  if (_0x5275x7 == 0) {
    ...
    if (_0x5275xc[_0x3c15[14]]) {
      ...
      return valStatus[_0x3c15[29]];
    }
  } else {
    return valStatus[_0x3c15[30]];
  }
} //d9-03
//d9-04
...
function getSymbolOverrides(symbolMaster, idsMap) {
  var symbolOverrides = []; // var instances = sketch.find("[type='SymbolInstance']", document);
...
function MergeSymbols(symbolToMerge, symbolToKeep, basePercent, totalToMerge, webContents) {
  ...
  for (var i = 0; i < symbolToMerge.duplicates.length; i++) {
    if (i != symbolToKeep) {
      ...
      var message = "Updating " + symbolToMerge.name; // var message2 = ((instancesOfSymbol.length > 0) ? ((instancesOfSymbol.length == 1) ? "1 instance" : instancesOfSymbol.length+" instances") : "");
...
function MergeSelectedTextStyles(context) {
  ...
  webContents.on('GetLocalStylesList', function () {
    Helpers.clog("Get local styles list");
    if (definedTextStyles == null) definedTextStyles = Helpers.getDefinedTextStyles(context, false);
    checkingAlsoLibraries = false;
    webContents.executeJavaScript("DrawStyleList(".concat(JSON.stringify(definedTextStyles), ")")).catch(console.error);
  });
  webContents.on('GetAllStylesList', function () {
    Helpers.clog("Get all (including libraries) styles list");
    if (definedAllTextStyles == null) definedAllTextStyles = Helpers.getDefinedTextStyles(context, true);
    checkingAlsoLibraries = true;
