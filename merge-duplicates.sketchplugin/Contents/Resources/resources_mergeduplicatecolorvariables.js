...
/******/ 	return __webpack_require__(__webpack_require__.s = "./resources/mergeduplicatecolorvariables.js");
/******/ })
/************************************************************************/
/******/ ({

/***/ "./resources/mergeduplicatecolorvariables.js":
/*!***************************************************!*\
  !*** ./resources/mergeduplicatecolorvariables.js ***!
  \***************************************************/
...
window.DrawColorVariablesList = function (mergeSession, includeLibraries) {
...
window.onSelectedStyleCheckChanged = function (index) {
  ...
  DrawColorVariablesList(globalMergeSession);
  DrawStyleList(globalStyleDisplayed);
};
...
window.ReDrawAfterGettingData = function (symbolData, index) {
  ...
  for (var i = 0; i < globalMergeSession[index].colorVariableWithDuplicates.duplicates.length; i++) {
    globalMergeSession[index].colorVariableWithDuplicates.duplicates[i].thumbnail = symbolData.duplicates[i].thumbnail;
  }
...
window.onStyleClicked = function (index, selectedStyle) {
  ...
  for (var i = 0; i < globalMergeSession[selectedStyle].colorVariableWithDuplicates.duplicates.length; i++) {
...
window.DrawStyleList = function (index) {
  ...
  for (var i = 0; i < globalMergeSession[index].colorVariableWithDuplicates.duplicates.length; i++) {
    window.postMessage("nativeLog", "WV --- Drawing style: " + globalMergeSession[index].colorVariableWithDuplicates.duplicates[i].name);
    ...
    var contrastMode = globalMergeSession[index].colorVariableWithDuplicates.duplicates[i].contrastMode ? "bgContrastMode" : "";
    inner += "<div id=\"duplicateItem".concat(i, "\" class=\"thumbnailContainer symbolPreview horizontalLayout alignVerticalCenter ").concat(selected, "\" onclick=\"onStyleClicked(").concat(i, ", ").concat(index, ")\">\n                ").concat(checkbox, "\n                <div class=\"colAvailable verticalLayout thumbnailData\" id=\"duplicateItemThumbnail").concat(i, "\" >\n                  <div class=\"rowAvailable padded ").concat(contrastMode, "\"><div class=\"thumbnail\" style='background-image:url(\"data:image/png;base64,").concat(globalMergeSession[index].colorVariableWithDuplicates.duplicates[i].thumbnail, "\")'></div></div>\n                  <div class=\"rowAuto primaryText displayFlex\"><span class=\"alignHorizontalCenter\">").concat(globalMergeSession[index].colorVariableWithDuplicates.duplicates[i].name, " (").concat(globalMergeSession[index].colorVariableWithDuplicates.duplicates[i].libraryName, ")</span></div>\n                  <div class=\"rowAuto secondaryText displayFlex\"><span class=\"alignHorizontalCenter\">").concat(globalMergeSession[index].colorVariableWithDuplicates.duplicates[i].description, "</span></div>\n                </div>\n              </div>");
  }
...
/******/ });
//# sourceMappingURL=resources_mergeduplicatecolorvariables.js.map